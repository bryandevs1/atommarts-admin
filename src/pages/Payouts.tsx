...
export default function AdminPayoutsDashboard() {
  ...
  const fetchPayouts = async () => {
    try {
      ...
      const response = await axios.get(
        `https://nexodus.tech/api/admin/payouts?${params.toString()}`,
        {
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-8">Loading payout requests...</div>
    );
  }
      ...
      <div className="flex flex-col mb-6 space-y-4 md:space-y-0 md:flex-row md:items-center md:justify-between dark:bg-white/[0.03]">
        ...
        <div className="flex space-x-4">
          ...
          <Input
            ...
            onChange={(e) =>
              setFilters({ ...filters, vendor_id: e.target.value, page: 1 })
            }
