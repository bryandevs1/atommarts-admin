  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(
        `https://nexodus.tech/api/vendor/vendors?limit=${itemsPerPage}&page=${currentPage}${
          searchTerm ? `&search=${searchTerm}` : ""
        }&role=vendor`, // Added role filter here
        {
          headers: {
                  ...
                  <TableCell>
                    {user.isApproved === 1 ? (
                      <div className="flex items-center gap-2">
                        <Badge color="success" className="text-theme-xs">
                          Approved
                        </Badge>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <Badge color="warning" className="text-theme-xs">
                          Pending
                        </Badge>
                      </div>
                    )}
